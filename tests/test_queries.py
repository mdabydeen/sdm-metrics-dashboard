...
class TestCapacityUpsert:
    def test_insert_new_capacity(self, conn):
        ...
        conn.commit()
        row = conn.execute(
            "SELECT * FROM sprint_capacity WHERE sprint_id=1 AND engineer_id='e1'"
        ).fetchone()
    ...
    def test_upsert_preserves_row_id(self, conn):
        ...
        conn.commit()
        row1 = conn.execute(
            "SELECT id FROM sprint_capacity WHERE sprint_id=1 AND engineer_id='e1'"
        ).fetchone()
        original_id = row1["id"]
        ...
        conn.commit()
        row2 = conn.execute(
            "SELECT id, available_days, notes FROM sprint_capacity WHERE sprint_id=1 AND engineer_id='e1'"
        ).fetchone()
    ...
    def test_upsert_preserves_entered_at(self, conn):
        ...
        conn.commit()
        row1 = conn.execute(
            "SELECT entered_at FROM sprint_capacity WHERE sprint_id=1 AND engineer_id='e1'"
        ).fetchone()
...
class TestMetricsUpsert:
    ...
    def test_upsert_preserves_row_id(self, conn):
        ...
        conn.commit()
        row1 = conn.execute(
            "SELECT id FROM sprint_metrics WHERE sprint_id=1 AND team_id='t1'"
        ).fetchone()
        original_id = row1["id"]
        ...
        conn.commit()
        row2 = conn.execute(
            "SELECT id, velocity FROM sprint_metrics WHERE sprint_id=1 AND team_id='t1'"
        ).fetchone()
